...
{
    public class AuthContext(IConfiguration configuration) : IAuthtContext
    {
        ...
        public async Task<ObjectListResponse<UserLogin>> GetUsersLogins()
        {
            ...
            try
            {
                ...
                if (ret == null)
                {
                    return new ObjectListResponse<UserLogin> { Success = false, ResultList = null, Errors = { new InternalError(eMessage.MSG_ERROR_REGISTER, "Houve algum problema na chamada") } };
                }
                else
                {
                    List<UserLogin> list = default;
                    int count = 0;
